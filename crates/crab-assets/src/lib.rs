...
fn slot_uv(slot: u32, grid: u32) -> [f32; 4] {
    ...
    // Leave a full texel of safety around each tile. Perspective
    // interpolation can move a nominal edge sample across an atlas boundary
    // even with nearest filtering; the extra half-texel margin prevents the
    // neighbouring block texture from bleeding into this face.
    let inset = step / TILE as f32;
...
mod tests {
    ...
    #[test]
    fn atlas_tiles_keep_a_bleed_safe_margin() {
        let uv = slot_uv(1, 4);
        let atlas_pixels = (4 * TILE) as f32;
        assert!((uv[0] * atlas_pixels - 17.0).abs() < 1e-5);
        assert!((uv[2] * atlas_pixels - 31.0).abs() < 1e-5);
    }
